...
time_line
  ...
  .fromTo(animation, 0.3, { opacity: 1 }, { opacity: 0 }); //opacity1到0是指過0.3秒後就會消失

// //setTimeout設定一個計時器 時間到後就做裡面的工作
setTimeout(() => {
  animation.style.pointerEvents = "none";
}, 2500); //2500是1+1+0.3=2.3
...
function setGPA() {
  ...
  if (creditSum == 0) {
    ...
    result = (sum / creditSum).toFixed(2); //.toFixed(2)取到小數後兩位
  }
...
function handleSorting(direction) {
  ...
  for (let i = 0; i < graders.length; i++) {
    ...
    let class_number = graders[i].children[1].value; //class category
    let class_credit = graders[i].children[2].value;
    let class_grade = graders[i].children[3].value;

    //只要以上四個其中任何一個有值，就會做class_object，且存進objectArray
    if (
      !(
        class_name == "" &&
        class_number == "" &&
  ...
  //創建一個新的object class_grade_number，放轉換後的class_grade
  for (let i = 0; i < objectArray.length; i++) {
    objectArray[i].class_grade_number = convertor(objectArray[i].class_grade);
  }

  //objectArray是由小到大排(ascending)
  //所以如果要用由大到小排的話(descending)，只需將ascending的結果反轉即可
  objectArray = merge_sort(objectArray);
  if (direction == "descending") {
    objectArray = objectArray.reverse();
  }
  //根據object array的內容更新網頁
  let allInputs = document.querySelector(".all_inputs");
  allInputs.innerHTML = ""; //清空all inputs表格的內容，因為要重放(排序)資料
  for (let i = 0; i < objectArray.length; i++) {
...
<div class="grader">
  ...
  //因為SELECT沒有辦法用設定VALUE的方式去更改，所以直接用JS改
  //graders要再重刷一次，因為是靜態的
